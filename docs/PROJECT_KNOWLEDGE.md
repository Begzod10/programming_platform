...
### 3.1 Alembic — squashed to a single baseline (2026-09-11)
`backend/alembic/versions/` is now a single revision, `badbe46c1225_baseline_consolidated_2026_09_11.py`
(`down_revision = None`), autogenerated against an empty scratch DB and hand-fixed for
the one circular FK in the schema (`groups.teacher_id` ↔ `students.group_id` — added
post-creation via `op.create_foreign_key`, since neither table can be created first
otherwise). Production's `alembic_version` is stamped at this revision. Before this fix
the chain had a duplicate revision id, a dangling `down_revision`, and — more
importantly — had **fully diverged across three untracked copies**: this repo's git
history, ~64 extra files that existed only on a local disk (untracked since the
2026-06-13 "never commit migrations again" `.gitignore` commit), and a completely
separate 60-file set on the production server itself (generated by running
`alembic revision --autogenerate` directly there, never committed). None of the three
matched. A dead, pre-`backend/`-restructure top-level `alembic/`+`alembic.ini` tree
(21 more files) was also removed — unreferenced by any code, deploy script, or CI step.

**Alembic is not part of the actual deploy pipeline** — confirmed by grep, neither
`deploy.sh` nor `webhook.py` on the server ever calls `alembic upgrade`. Schema changes
reach production exclusively through `init_db()`'s `Base.metadata.create_all()` (new
tables only — idempotent, never alters existing ones) plus the hand-maintained
`_reconcile_indexes()` idempotent-DDL list in `database.py` (constraints/indexes added
to tables that already existed in prod). Treat the alembic tree as documentation of
intended schema plus a working local dev tool, not as the live source of truth for
prod — verify against the actual DB (`\d <table>` via psql) before assuming a migration
file's changes are actually applied there. Several tables (`video_watches`,
`lesson_completions`, `exercise_submissions`, `exercises`, `course_certificates`,
`quizzes`, `questions`, `student_quiz_results`) still have **no real migration coverage**
in the sense that they only exist in prod because of `create_all()`, not because any
migration ran — this is fine as long as `create_all()` keeps being the actual mechanism,
but means a genuinely new column on an *existing* table still needs a manual
`_reconcile_indexes()`-style statement (for constraints/indexes) or direct DDL, since
`create_all()` won't add it and alembic won't either unless someone starts running it
for real.

Known **pre-existing schema drift** surfaced while verifying the new baseline against
prod (`alembic check` / raw `compare_metadata`), not fixed as part of this cleanup —
see BACKEND_BUGS.md for detail: `quizzes` has 3 dead columns from a reverted feature,
and `student_question_orders` is missing the unique constraint its own (never-applied)
migration intended, so duplicate-order protection may not actually be enforced live.
