...
- **✅ Fixed (2026-09-11):** squashed `backend/alembic/versions/` (12 tracked files with a broken chain — duplicate revision id `aa11bb22cc33`, a dangling `down_revision` to a deleted file — plus 64 more stale files that existed only on disk, never in git) into one clean baseline `badbe46c1225_baseline_consolidated_2026_09_11.py`, generated via `alembic revision --autogenerate` against an empty scratch DB and hand-fixed for the `groups`↔`students` circular FK (Alembic's autogenerate can't order mutually-dependent tables; the FK is now added via a standalone `op.create_foreign_key` after both tables exist, with the matching `op.drop_constraint` in `downgrade()`). Verified with a full `upgrade → downgrade → upgrade` round-trip and `alembic check` (zero drift vs. current models) against the scratch DB before touching production. Production's `alembic_version` was stamped (bookkeeping-only, no DDL: `alembic stamp --purge badbe46c1225`) to the new baseline. Also deleted a completely dead, pre-`backend/`-restructure top-level `alembic/`+`alembic.ini` tree (21 more files, unreferenced by any code, deploy script, or CI workflow) and the production server's own out-of-sync `backend/alembic/versions/` (60 auto-generated files that were never committed to git at all — someone had run `alembic revision --autogenerate` directly on the server). `env.py`/`script.py.mako`/`README` also existed only on the server, never in git, until now.
- **🟡 Found but intentionally not touched in this fix — pre-existing production schema drift, unrelated to any change this session:** live `quizzes` table has 3 extra columns (`grade_min`, `grade_max`, `course_id` + its FK to `courses`) not present in the current `Quiz` model — dead columns from a since-reverted feature on the legacy quiz system (see §3.1 in PROJECT_KNOWLEDGE.md on the three coexisting "quiz" concepts). More importantly: `student_question_orders` has a plain non-unique index `ix_student_question_orders_session_student` in production, not the `UniqueConstraint(session_id, student_id)` the model and the (never-applied) `ee11ff22aa33_add_unique_constraint_student_question_order` migration both call for — meaning the duplicate-order protection that migration was written for is likely **not actually enforced on production today**. Flagged for a follow-up ALTER TABLE, out of scope for alembic-tree cleanup itself.
