"""Refresh the existing 'HTML CSS' course (id ~9) to match seed_html_css_course.py.

Mirrors refresh_javascript_text.py: matches exercises by title (every existing
exercise has order=0), reassigns sequential `order`, inserts new exercises
(L3 +2, L8 +1), and updates lesson/exercise text fields without touching
student submissions.

Usage:
    cd backend
    python scripts/refresh_html_css_text.py            # dry-run (default)
    python scripts/refresh_html_css_text.py --apply    # actually write
"""
...
from scripts.seed_html_css_course import (  # noqa: E402
...
async def refresh(apply: bool) -> None:
    ...
    print(f"=== HTML CSS Refresh ({label}) ===\n")
    ...
    async with AsyncSessionLocal() as db:
        ...
        if not course:
            raise SystemExit(
                f"Course {COURSE['title']!r} not found. Run seed_html_css_course.py first."
            )
        ...
        for ldata in LESSONS:
            ...
            if lesson_changes or ex_changes_count or ex_inserts_count or order_reassigns:
            ...
            if apply and (lesson_changes or ex_changes_count or ex_inserts_count or order_reassigns):
                await db.flush()
                fresh = {
                    e.id: e for e in (
                        await db.execute(
                            select(Exercise).where(Exercise.lesson_id == lesson.id).order_by(Exercise.order, Exercise.id)
                        )
                    ).scalars().all()
                }
                ordered = sorted(fresh.values(), key=lambda x: (x.order, x.id))
        ...
        if not (total_lesson_fields or total_ex_field_updates or total_ex_inserts):
