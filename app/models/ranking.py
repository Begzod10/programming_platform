...
class Ranking(Base):
    ...
    # Reset va hisob-kitob vaqtlari
    last_calculated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(),
                                                         nullable=False)
    last_daily_reset: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(),
                                                       nullable=False)
    last_weekly_reset: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(),
                                                        nullable=False)
    last_monthly_reset: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(),
                                                         nullable=False)
