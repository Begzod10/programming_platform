...
from app.schemas.degree import DegreeCreate, DegreeUpdate, DegreeRead, StudentDegreeRead, DegreeProgress, \
    CertificateVerify
...
async def my_degrees(student_id: int = Query(...), db: AsyncSession = Depends(get_db)):
    ...
    return [
        StudentDegreeRead(id=sd.id, degree_name=sd.degree.name, degree_level=sd.degree.level, earned_at=sd.earned_at,
                          verification_code=sd.verification_code, certificate_url=sd.certificate_url) for sd in degrees]
...
async def verify_certificate(verification_code: str, db: AsyncSession = Depends(get_db)):
    ...
    return CertificateVerify(valid=True, student_name=cert.student.full_name, username=cert.student.username,
                             degree_name=cert.degree.name, degree_level=cert.degree.level, earned_at=cert.earned_at,
...
async def create_degree(data: DegreeCreate, db: AsyncSession = Depends(get_db)):
    return await degree_service.create_degree(db, data.name, data.description, data.level, data.required_points,
                                              data.required_projects, data.certificate_template, data.badge_image_url)
...
async def update_degree(degree_id: int, data: DegreeUpdate, db: AsyncSession = Depends(get_db)):
    degree = await degree_service.update_degree(db, degree_id, name=data.name, description=data.description,
...
@router.post("/award")
async def award_degree(student_id: int = Query(...), degree_id: int = Query(...), db: AsyncSession = Depends(get_db)):
...
async def check_and_award(student_id: int = Query(...), db: AsyncSession = Depends(get_db)):
    ...
    return {"message": f"{len(awarded)} ta yangi daraja berildi!",
