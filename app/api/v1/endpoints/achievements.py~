...
from app.schemas.achievement import AchievementCreate, AchievementUpdate, AchievementRead, StudentAchievementRead, AchievementProgress
...
async def my_achievements(student_id: int = Query(...), db: AsyncSession = Depends(get_db)):
    ...
    return [StudentAchievementRead(id=sa.id, achievement_name=sa.achievement.name, description=sa.achievement.description, badge_image_url=sa.achievement.badge_image_url, points_reward=sa.achievement.points_reward, earned_at=sa.earned_at) for sa in achievements]
...
async def create_achievement(data: AchievementCreate, db: AsyncSession = Depends(get_db)):
    return await achievement_service.create_achievement(db, data.name, data.description, data.badge_image_url, data.points_reward, data.criteria_type, data.criteria_value)
...
async def update_achievement(achievement_id: int, data: AchievementUpdate, db: AsyncSession = Depends(get_db)):
    achievement = await achievement_service.update_achievement(db, achievement_id, name=data.name, description=data.description, badge_image_url=data.badge_image_url, points_reward=data.points_reward, criteria_type=data.criteria_type, criteria_value=data.criteria_value)
    if not achievement:
...
@router.post("/award")
async def award_achievement(student_id: int = Query(...), achievement_id: int = Query(...), db: AsyncSession = Depends(get_db)):
...
async def check_and_award(student_id: int = Query(...), db: AsyncSession = Depends(get_db)):
    ...
    return {"message": f"{len(awarded)} ta yangi achievement berildi!", "awarded": [{"id": a.achievement_id, "earned_at": a.earned_at} for a in awarded]}