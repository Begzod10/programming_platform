...
const RichTextEditor = ({value, onChange}) => {
    ...
    const restore = () => {
        editorRef.current.focus();                          // ← avval focus
        ...
        <div>
            <div className="rte-toolbar" onMouseDown={prevent}>
                ...
                <select className="rte-select" defaultValue="14px" onMouseDown={e => e.stopPropagation()}
                        onChange={e => {
                            restore();
                            // execCommand fontSize faqat 1-7 qabul qiladi, shuning uchun span ishlatamiz
                            document.execCommand('fontSize', false, '7');
                            // Yaratilgan font teglarni span ga aylantiramiz
                ...
                <button className="rte-btn" onMouseDown={prevent} onClick={() => exec('strikeThrough')}><s>S</s>
                </button>
                <div className="rte-divider"/>
                {/* Align */}
                ...
                <div className="rte-divider"/>
                {/* Lists */}
                <button className="rte-btn" onMouseDown={prevent} onClick={() => exec('insertUnorderedList')}>• –
                </button>
                ...
                <div className="rte-color-wrap" title="Цвет текста" onMouseDown={prevent}
                    ...
                    <div className="rte-color-dot" style={{
                        background: '#1a1a2e',
                        display: 'flex',
                        alignItems: 'center',
...
const ExerciseItem = ({ex, index, onChange, onDelete}) => {
    ...
    const addOption = () => {
        ...
        if (!v) return;
        upd({options: [...optionsList, v].join(',')});
        setOptionInput('');
    };
    ...
    const addDrag = () => {
        const v = dragInput.trim();
        if (!v) return;
        upd({drag_items: [...dragList, v].join(',')});
        setDragInput('');
    };
        ...
        <div className="ex-item">
            ...
            <div className="ex-item-body">
                    ...
                    <div className="ex-meta-group">
                        ...
                        <div className="ex-diff-pills">
                            {DIFFICULTY_LEVELS.map(d => (
                                <button key={d.v}
                                        className={`ex-diff-pill ${ex.difficulty_level === d.v ? 'active' : ''}`}
                                        style={ex.difficulty_level === d.v ? {
                                            background: d.color,
                                            color: '#fff',
                                            borderColor: d.color
                                        } : {}}
                ...
                {ex.exercise_type === 'text_input' && (<>
                    ...
                    <div className="lm-field">
                        <label>Ожидаемый ответ <span
                            className="ex-label-note">(AI будет проверять по смыслу)</span></label>
                        <textarea rows={2} placeholder="Опишите правильный ответ..."
                                  value={ex.expected_answer} onChange={e => upd({expected_answer: e.target.value})}/>
                    </div>
                </>)}
                ...
                {ex.exercise_type === 'multiple_choice' && (<>
                    ...
                    <div className="lm-field">
                        ...
                        <div className="ex-options-list">
                            ...
                            {optionsList.length === 0 &&
                                <div className="ex-options-empty">Добавьте хотя бы 2 варианта</div>}
                        </div>
                    ...
                    <div className="lm-field">
                        <label>Правильный(е) ответ(ы) * <span
                            className="ex-label-note">— напишите букву(ы): A или A,C</span></label>
                ...
                {ex.exercise_type === 'drag_and_drop' && (<>
                    ...
                    <div className="lm-field">
                        ...
                        <div className="ex-drag-list">
                            ...
                            {dragList.length === 0 &&
                                <div className="ex-options-empty">Добавьте элементы которые студент будет
                                    перетаскивать</div>}
                        </div>
                        <div className="ex-input-row">
                            <input placeholder="Новый элемент..." value={dragInput}
                                   onChange={e => setDragInput(e.target.value)}
                                   onKeyDown={e => e.key === 'Enter' && addDrag()}/>
                ...
                {ex.exercise_type === 'fill_in_blank' && (<>
                    <div className="lm-field">
                        <label>Текст с пропусками * <span
                            className="ex-label-note">— используйте ___ для пропусков</span></label>
                            ...
                            <div className="ex-blank-preview-text">
                                {ex.description.split('___').map((part, i, arr) => (
                                    <span key={i}>{part}{i < arr.length - 1 &&
                                        <span className="ex-blank-input-mock"/>}</span>
                                ))}
                            </div>
                        </div>
                    )}
                    <div className="lm-field">
                        <label>Правильные ответы * <span
                            className="ex-label-note">— через запятую, по порядку пропусков</span></label>
...
const ExerciseEditor = ({section, onUpdate}) => {
    ...
    const updateExercise = (localId, data) => onUpdate({
        ...section,
        exercises: exercises.map(e => e._localId === localId ? data : e)
    });
            ...
            <div className="lm-section-body">
                ...
                {section.type === 'video' && (<>
                    ...
                    <div className="lm-video-preview">
                        {ytId ? <iframe src={`https://www.youtube.com/embed/${ytId}`} allowFullScreen title="preview"/>
                            : <div className="lm-video-placeholder">
                                <div className="lm-video-placeholder-icon">🎬</div>
                                <p>Введите YouTube ссылку</p></div>}
                    </div>
                </>)}
                {section.type === 'image' && (<>
                    ...
                    {section.imgUrl &&
                        <div className="lm-img-preview"><img src={section.imgUrl} alt={section.imgName || ''}/></div>}
                ...
                {section.type === 'file' && (<>
                    <div className="lm-file-drop" onClick={() => fileRef.current.click()}>
                        <div className="lm-file-drop-icon">📁</div>
                    ...
                    {section.fileName && (
                        <div className="lm-file-info">
                            ...
                            <div>
                                <div className="lm-file-name">{section.fileName}</div>
                                <div className="lm-file-size">{section.fileSize}</div>
                            </div>
                        </div>
                    )}
                </>)}
                    ...
                    <div className="lm-project-fields">
                        ...
                        <div className="lm-field"><label>Требования</label>
                            <textarea className="lm-project-textarea" rows={4} placeholder="1. ...\n2. ..."
                                      ...
                                      onChange={e => update({requirements: e.target.value})}/></div>
...
const LessonModal = ({course, lesson, chapters, onSave, onClose}) => {
    ...
    useEffect(() => {
        ...
        return () => {
            if (p) p.classList.remove('stop-scrolling');
        };
    }, []);
    ...
    return ReactDOM.createPortal(
        <div className="lm-overlay" onClick={e => {
