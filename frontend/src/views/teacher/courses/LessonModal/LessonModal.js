...
/* ─────────────────────────────────────────
   Rich Text Editor — РАБОЧИЙ ВАРИАНТ
───────────────────────────────────────── */
const RichTextEditor = ({value, onChange}) => {
    ...
    const restore = () => {
        editorRef.current.focus(); // сначала фокус, потом восстановление
    ...
    // prevent — блокирует потерю фокуса у редактора при клике на тулбар
    ...
    return (
        <div>
            <div className="rte-toolbar" onMouseDown={prevent}>
                ...
                {/* Heading */}
                <select className="rte-select" defaultValue="Paragraph"
                ...
                {/* Font family */}
                <select className="rte-select" defaultValue="Inter"
                        onMouseDown={e => e.stopPropagation()}
                        onChange={e => {
                            restore();
                ...
                {/* Font size — через fontSize 7 + замена на span */}
                <select className="rte-select" defaultValue="14px"
                        onMouseDown={e => e.stopPropagation()}
                        onChange={e => {
                            restore();
        ...
        <>
            ...
            <div className="lm-sidebar-section">
                ...
                <div className="lm-type-grid">
                    {SECTION_TYPES.map(t => (
                        <button key={t.type} className="lm-type-card" onClick={() => {
                            addSection(t.type);
    ...
    return (
        <div className="lm-page">
            ...
            <div className="lm-page-body">
                <div className={`lm-sidebar-overlay ${sidebarOpen ? 'open' : ''}`}
                     onClick={() => setSidebarOpen(false)}/>

                <div ref={sidebarRef} className={`lm-page-sidebar ${sidebarOpen ? 'mobile-open' : ''}`}>
                    <SidebarContent/>
                </div>
            ...
            <button className="lm-sidebar-toggle" onClick={() => setSidebarOpen(o => !o)} title="Меню">
                {sidebarOpen ? '✕' : '✚'}
            </button>
        </div>
    );
