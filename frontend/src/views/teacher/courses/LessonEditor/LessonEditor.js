...
const RichTextEditor = ({ value, onChange }) => {
    ...
    const applyFontSize = (e) => {
        ...
        if (savedRange.current && !savedRange.current.collapsed) {
            ...
            if (fonts.length) { const last = fonts[fonts.length-1]; last.removeAttribute('size'); last.style.fontSize = size; }
            onChange(editorRef.current.innerHTML);
        }
        editorRef.current.focus();
    };
        ...
        <div className="lep-rte-wrap">
            <div className="lep-rte-toolbar" onMouseDown={preventBlur}>
                ...
                <div className="lep-rte-color" title="Цвет текста">
                    ...
                    <input type="color" className="lep-rte-color-input" defaultValue="#1a1a2e" onMouseDown={e=>e.stopPropagation()} onChange={e=>applyColor(e,'foreColor')}/>
                </div>
                <div className="lep-rte-color" title="Фон текста">
                    <div className="lep-rte-dot" style={{background:'#a29bfe'}}/>
                    <input type="color" className="lep-rte-color-input" defaultValue="#a29bfe" onMouseDown={e=>e.stopPropagation()} onChange={e=>applyColor(e,'hiliteColor')}/>
                </div>
                <div className="lep-rte-sep"/>
        ...
        <div className="lep-ex-row">
            <div className="lep-ex-row-head">
                <div className="lep-ex-row-left">
                    ...
                    <select className="lep-ex-diff-sel" value={ex.difficulty_level} onChange={e=>upd({difficulty_level:e.target.value})}>
                        {DIFF_LEVELS.map(d=><option key={d}>{d}</option>)}
                    </select>
                    <div className="lep-ex-pts-wrap">
                        <input className="lep-ex-pts-input" type="number" value={ex.points} onChange={e=>upd({points:Number(e.target.value)})} title="Баллы"/>
                ...
                <div className="lep-ex-row-body">
                    <div className="lep-ex-field-row">
                        <div className="lep-ex-field">
                            ...
                            <input className="lep-ex-input" value={ex.title} onChange={e=>upd({title:e.target.value})} placeholder="Заголовок..."/>
                        </div>
                    </div>
                    ...
                    {ex.exercise_type !== 'fill_in_blank' && (
                        <div className="lep-ex-field">
                            ...
                            <textarea className="lep-ex-textarea" value={ex.description} onChange={e=>upd({description:e.target.value})} placeholder="Опишите задание..."/>
                        </div>
                    )}
                    ...
                    {ex.exercise_type === 'multiple_choice' && (<>
                        <div className="lep-ex-field">
                            ...
                            <input className="lep-ex-input" value={ex.options} onChange={e=>upd({options:e.target.value})} placeholder="Вариант A, Вариант B, Вариант C"/>
                        </div>
                        <div className="lep-ex-field">
                            <label>Правильный ответ(ы) <span className="lep-ex-hint-label">(через запятую)</span></label>
                            <input className="lep-ex-input" value={ex.correct_answers} onChange={e=>upd({correct_answers:e.target.value})} placeholder="Вариант A"/>
                        </div>
                    ...
                    {ex.exercise_type === 'drag_and_drop' && (<>
                        <div className="lep-ex-field">
                            ...
                            <input className="lep-ex-input" value={ex.drag_items} onChange={e=>upd({drag_items:e.target.value})} placeholder="Элемент 1, Элемент 2, Элемент 3"/>
                        </div>
                        <div className="lep-ex-field">
                            <label>Правильный порядок <span className="lep-ex-hint-label">(через запятую)</span></label>
                            <input className="lep-ex-input" value={ex.correct_order} onChange={e=>upd({correct_order:e.target.value})} placeholder="Элемент 1, Элемент 3, Элемент 2"/>
                        </div>
                    </>)}
                    ...
                    {ex.exercise_type === 'fill_in_blank' && (<>
                        <div className="lep-ex-field">
                            ...
                            <textarea className="lep-ex-textarea" value={ex.description} onChange={e=>upd({description:e.target.value})} placeholder="Столица России — ___, а Франции — ___."/>
                        </div>
                        <div className="lep-ex-field">
                            <label>Правильные ответы <span className="lep-ex-hint-label">(через запятую, по порядку)</span></label>
                            <input className="lep-ex-input" value={ex.correct_answers} onChange={e=>upd({correct_answers:e.target.value})} placeholder="Москва, Париж"/>
                        </div>
                    </>)}
                    ...
                    {ex.exercise_type === 'text_input' && (
                        <div className="lep-ex-field">
                            ...
                            <input className="lep-ex-input" value={ex.expected_answer} onChange={e=>upd({expected_answer:e.target.value})} placeholder="Ключевые слова или суть правильного ответа"/>
                        </div>
                    )}

                    <div className="lep-ex-field">
                        <label>Подсказка <span className="lep-ex-hint-label">(необязательно)</span></label>
                        <input className="lep-ex-input" value={ex.hint} onChange={e=>upd({hint:e.target.value})} placeholder="Подсказка для студента..."/>
                    </div>
                </div>
            )}
        </div>
            ...
            {!collapsed && (
                <div className="lep-section-body">
                    ...
                    {section.type === 'code' && (<>
                        ...
                        <textarea className="lep-code-area" placeholder="// Вставьте код сюда..." value={section.code} onChange={e=>update({code:e.target.value})}/>
                    </>)}

                    {section.type === 'video' && (<>
                        <input className="lep-input" placeholder="YouTube ссылка (https://youtu.be/...)" value={section.videoUrl} onChange={e=>update({videoUrl:e.target.value})}/>
                    ...
                    {section.type === 'file' && (<>
                        ...
                        {section.fileName && (
                            <div className="lep-file-info">
                                <span>📦</span>
                                <div><div className="lep-file-name">{section.fileName}</div><div className="lep-file-size">{section.fileSize}</div></div>
                            </div>
                        )}
                    </>)}
                    ...
                    {section.type === 'project' && (
                        <div className="lep-project-editor">
                            <div className="lep-ex-field">
                                ...
                                <textarea className="lep-ex-textarea" value={section.description||''} onChange={e=>update({description:e.target.value})} placeholder="Опишите суть проекта..."/>
                            </div>
                            <div className="lep-ex-field">
                                <label>Требования</label>
                                <textarea className="lep-ex-textarea" value={section.requirements||''} onChange={e=>update({requirements:e.target.value})} placeholder="Перечислите требования к проекту..."/>
                            </div>
                            <div className="lep-ex-field-row">
                                <div className="lep-ex-field">
                                    <label>Стек технологий <span className="lep-ex-hint-label">(через запятую)</span></label>
                                    <input className="lep-ex-input" value={section.techStack||''} onChange={e=>update({techStack:e.target.value})} placeholder="React, Node.js, PostgreSQL"/>
                                </div>
                                <div className="lep-ex-field lep-ex-field--sm">
                                    <label>Дедлайн (дней)</label>
                                    <input className="lep-ex-input" type="number" value={section.deadline||''} onChange={e=>update({deadline:e.target.value})} placeholder="7"/>
                                </div>
                            </div>
                            {section.techStack && (
                                    ...
                                    {(section.exercises||[]).map((ex,i)=>{
                                        ...
                                        return (
                                            <ExerciseRow key={lid} ex={ex} index={i}
                                                isFirst={i===0} isLast={i===(section.exercises||[]).length-1}
                    ...
                    {section.type === 'mashq' && (
                        <div className="lep-project-editor">
                            <div className="lep-ex-field">
                                ...
                                <select className="lep-ex-input" value={section.mashqType||'textarea'} onChange={e=>update({mashqType:e.target.value})}>
                            ...
                            <div className="lep-ex-field">
                                <label>Вопрос</label>
                                <textarea className="lep-ex-textarea" value={section.question||''} onChange={e=>update({question:e.target.value})} placeholder="Введите вопрос..."/>
                            </div>
                            <div className="lep-ex-field">
                                <label>Правильный ответ</label>
                                <input className="lep-ex-input" value={section.answer||''} onChange={e=>update({answer:e.target.value})} placeholder="Ответ..."/>
                            </div>
                    ...
                    <div className="lep-sidebar-card">
                        ...
                        <div className="lep-field">
                            ...
                            <input className={`lep-field-input${!form.title.trim()?' lep-field-input--error':''}`}
                                placeholder="Введите название..." value={form.title} onChange={e=>setField('title',e.target.value)}/>
                            {!form.title.trim() && <span className="lep-field-hint">Обязательное поле</span>}
                        </div>
                        <div className="lep-field">
                            <label>Раздел / Глава</label>
                            <select className="lep-field-select" value={form.chapter} onChange={e=>setField('chapter',e.target.value)}>
                        ...
                        <div className="lep-field">
                            ...
                            <input className="lep-field-input" placeholder="https://..." value={form.image} onChange={e=>setField('image',e.target.value)}/>
                            {form.image && (
                    ...
                    {form.sections.length > 0 && (
                        <div className="lep-sidebar-card lep-stats-card">
                            ...
                            {[
                                ...
                                return cnt>0?<div key={type} className="lep-stat-row"><span>{icon} {label}</span><strong>{cnt}</strong></div>:null;
                            })}
                        </div>
                    )}
                </aside>
